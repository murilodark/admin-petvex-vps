...
export const PlansPage: React.FC = () => {
  ...
  // Layout view style: Default to 'grid' to showcase beautiful pricing cards
  ...
  // Debounced search trigger to ensure we don't duplicate rendering queries on page mount
  ...
  // Combined cohesive query loop that triggers exactly once on state updates
  useEffect(() => {
    let active = true;
  ...
  const handleSearchChange = (val: string) => {
    setSearch(val);
    setPage(1); // Set page immediately on typing
  };

  const handleStatusChange = (val: string) => {
    setStatus(val);
    setPage(1); // Set page immediately on filter select
  };
  ...
  const handleFormSubmit = async (formData: PlanFormData) => {
    ...
    try {
      if (selectedPlan) {
        // Edit Mode
        await plansService.atualizarPlano(selectedPlan.id, formData);
        triggerToast(`Configurações de precificação do plano "${formData.name}" gravadas com sucesso!`, 'success');
      } else {
        // Create Mode
        await plansService.cadastrarPlano(formData);
        triggerToast(`O plano "${formData.name}" foi configurado e adicionado à grade SaaS!`, 'success');
      }
      setIsModalOpen(false);
      loadPlans(debouncedSearch);
    } catch (err) {
      throw err; // Forward to modal form so it prints validation block alerts
    } finally {
  ...
  const averagePrice = rawData.length > 0 
    ? rawData.reduce((acc, curr) => acc + (curr.monthly_price || 0), 0) / rawData.length 
    : 0;
  ...
  return (
    <div className="space-y-6" id="plans-page-container">
      ...
      {/* Main Title toolbar */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white border border-slate-200 p-6 rounded-[4px]" id="plans-module-toolbar">
        <div className="text-left">
          ...
          <p className="text-[10px] text-slate-400 font-mono uppercase mt-1.5 pl-9">
            Defina planos comerciais, limites operacionais, valores de faturamento e ofertas de mercado do ecossistema Petvex
          </p>
        </div>
      ...
      {/* Stats Widgets Panel - High Level UX styling */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4" id="plans-numerical-stats-panel">
        ...
        <div className="bg-white border border-slate-200 rounded-[4px] p-5 flex flex-col justify-between shadow-xs text-left">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-mono font-black text-slate-400 uppercase tracking-widest">PLANOS CADASTRADOS</span>
            <div className="w-6 h-6 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 text-xs">#</div>
          </div>
          <div className="mt-3">
            <span className="text-2xl font-black text-slate-900 leading-none">{totalPlansCount}</span>
            <span className="block text-[9px] text-slate-400 font-bold uppercase mt-1 font-mono">MODELOS CONFIGURADOS</span>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-[4px] p-5 flex flex-col justify-between shadow-xs text-left">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-mono font-black text-slate-400 uppercase tracking-widest">OFERTAS EM ATIVIDADE</span>
            <div className="w-6 h-6 rounded-full bg-teal-50 border border-teal-100 flex items-center justify-center text-teal-500 text-xs">●</div>
          </div>
          <div className="mt-3">
            <span className="text-2xl font-black text-slate-900 leading-none">{activePlansCount}</span>
            <span className="block text-[9px] text-teal-600 font-bold uppercase mt-1 font-mono">DISPONÍVEL P/ ASSINATURAS</span>
          </div>
        </div>

        {/* Card 3 - Average pricing rate */}
        <div className="bg-white border border-slate-200 rounded-[4px] p-5 flex flex-col justify-between shadow-xs text-left">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-mono font-black text-slate-400 uppercase tracking-widest">TICKET SAAS MÉDIO</span>
            <span className="text-[9px] font-black uppercase text-indigo-500 bg-indigo-50 px-1 border border-indigo-100 rounded">R$</span>
          </div>
          <div className="mt-3">
            <span className="text-2xl font-black text-slate-900 leading-none">
              {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(averagePrice).split(',')[0]}
            </span>
            <span className="block text-[9px] text-slate-400 font-bold uppercase mt-1 font-mono font-bold">VALOR MENSAL MÉDIO</span>
          </div>
        </div>

        {/* Card 4 - Monetization standard banner */}
        <div className="bg-slate-900 text-slate-100 rounded-[4px] p-5 flex flex-col justify-between border border-slate-950 relative overflow-hidden text-left">
          ...
          <div className="flex items-center justify-between relative z-1">
            <span className="text-[9px] font-mono font-black text-teal-400 uppercase tracking-widest font-bold">COMPLIANCE DE RECEITA</span>
          ...
          <div className="mt-3 relative z-1">
            ...
            <span className="block text-[9px] text-slate-400 font-bold uppercase mt-0.5 font-mono">PREÇOS SINCRONIZADOS</span>
          </div>
        </div>
      ...
      {/* Safe confirmation modal for deletes */}
      {deleteTarget ? (
            ...
            <div className="flex gap-4 items-start">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-slate-500 leading-relaxed font-semibold">
                  Tem certeza que deseja excluir permanentemente o modelo <strong className="text-slate-900">{deleteTarget.name}</strong>? Esta operação removerá a licença das tabelas de contratação pública. Clientes já associados a esse plano manterão as faturas sob contrato legado.
                </p>
              </div>
            </div>
