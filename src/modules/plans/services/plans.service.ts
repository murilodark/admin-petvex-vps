...
export const plansService = {
  ...
  async cadastrarPlano(formData: PlanFormData): Promise<Plan> {
    const apiPayload: StorePlanRequest = plansMapper.toApiCreate(formData) as StorePlanRequest;
  ...
  async atualizarPlano(id: string | number, formData: Partial<PlanFormData>): Promise<Plan> {
    const stringId = id.toString();
    const apiPayload: UpdatePlanRequest = plansMapper.toApiUpdate(formData) as UpdatePlanRequest;
