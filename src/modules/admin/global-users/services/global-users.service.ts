import {
  ...
  getAdminUsersUser,
  putAdminUsersUser,
  deleteAdminUsersUser,
  getAdminUsersUserTenants,
  postAdminUsersUserTenants,
  putAdminUsersUserTenantsTenant,
  deleteAdminUsersUserTenantsTenant
...
export const globalUsersService = {
  async listarUsuarios(params: ListarGlobalUsersParams = {}): Promise<ListarGlobalUsersResult> {
    try {
      ...
      const response = await getAdminUsers(cleanParams);

      return {
        data: response.data.map((u: any) => globalUserMapper.toUiUser(u)),
  ...
  async buscarUsuarioPorId(userId: string): Promise<GlobalUser> {
    try {
      const response = await getAdminUsersUser(userId);
      return globalUserMapper.toUiUser(response);
    } catch (e) {
  ...
  async cadastrarUsuario(formData: GlobalUserFormData): Promise<GlobalUser> {
    ...
    try {
      const response = await postAdminUsers(payload);
      const mapped = globalUserMapper.toUiUser(response);
  ...
  async atualizarUsuario(userId: string, formData: Partial<GlobalUserFormData>, loggedInUserId?: string): Promise<GlobalUser> {
    ...
    try {
      const response = await putAdminUsersUser(userId, payload);
      const mapped = globalUserMapper.toUiUser(response);
  ...
  async excluirUsuario(userId: string, loggedInUserId?: string): Promise<void> {
    ...
    try {
      await deleteAdminUsersUser(userId);
  ...
  async listarVinculosTenants(userId: string): Promise<GlobalUserTenantAccess[]> {
    try {
      const response = await getAdminUsersUserTenants(userId);
      // userTenantAccessListResponse structure contains { data: UserTenantAccess[] }
      return response.data?.map(access => globalUserMapper.toUiTenantAccess(access, userId)) || [];
    } catch (e) {
  ...
  async vincularTenant(userId: string, formData: GlobalUserTenantAccessFormData): Promise<GlobalUserTenantAccess> {
    ...
    try {
      const response = await postAdminUsersUserTenants(userId, payload);
      const mapped = globalUserMapper.toUiTenantAccess(response, userId);
  ...
  async atualizarVinculoTenant(userId: string, tenantId: string, formData: Partial<GlobalUserTenantAccessFormData>): Promise<GlobalUserTenantAccess> {
    ...
    try {
      const response = await putAdminUsersUserTenantsTenant(userId, tenantId, payload);
      const mapped = globalUserMapper.toUiTenantAccess(response, userId);
  ...
  async desvincularTenant(userId: string, tenantId: string): Promise<void> {
    ...
    try {
      await deleteAdminUsersUserTenantsTenant(userId, tenantId);
