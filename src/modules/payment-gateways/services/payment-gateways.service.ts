import {
  activateAdminBillingPaymentGatewayActivate,
  createAdminBillingPaymentGateway,
  deactivateAdminBillingPaymentGatewayDeactivate,
  deleteAdminBillingPaymentGateway,
  getAdminBillingPaymentGateway,
  listAdminBillingPaymentGateways,
  setDefaultAdminBillingPaymentGatewaySetDefault,
...
export const paymentGatewaysService = {
  async listarGateways(params: ListarPaymentGatewaysParams = {}): Promise<ListarPaymentGatewaysResult> {
    try {
      ...
      const response = (await listAdminBillingPaymentGateways(cleanParams)) as any;
  ...
  async buscarGatewayPorId(id: string): Promise<PaymentGateway> {
    try {
      const response = await getAdminBillingPaymentGateway(Number(id));
      const resAny = response as any;
      const entity = resAny?.data?.data ?? resAny?.data ?? response;
      return entity as PaymentGateway;
    } catch (e) {
  ...
  async cadastrarGateway(formData: PaymentGatewayFormData): Promise<PaymentGateway> {
    ...
    try {
      const response = await createAdminBillingPaymentGateway(payload as any);
      const resAny = response as any;
  ...
  async atualizarGateway(id: string, formData: PaymentGatewayFormData): Promise<PaymentGateway> {
    ...
    try {
      const response = await updateAdminBillingPaymentGateway(Number(id), payload as any);
      const resAny = response as any;
      const result = (resAny && resAny.data) ? resAny.data : response;
  ...
  async testarConexao(id: string): Promise<{ success: boolean; message: string }> {
    try {
      const response = await testAdminBillingPaymentGatewayTest(Number(id));
      const resAny = response as any;
  ...
  async ativarGateway(id: string): Promise<PaymentGateway> {
    try {
      const response = await activateAdminBillingPaymentGatewayActivate(Number(id));
      const resAny = response as any;
      const result = (resAny && resAny.data) ? resAny.data : response;
  ...
  async desativarGateway(id: string): Promise<PaymentGateway> {
    try {
      const response = await deactivateAdminBillingPaymentGatewayDeactivate(Number(id));
      const resAny = response as any;
      const result = (resAny && resAny.data) ? resAny.data : response;
